"""Access control integration with Omni MCP module.

This module provides integration with the Omni MCP module's access control
...
class AccessController:
    """Controls access to Omni models via MCP module REST API."""
    ...
    def __init__(self, config: OmniConfig, cache_ttl: int = CACHE_TTL):
        """Initialize access controller.

        Args:
            config: OmniConfig with connection details and API key
        ...
        if not config.api_key:
            raise AccessControlError(
                "API key required for access control. Please configure OMNI_API_KEY."
            )
