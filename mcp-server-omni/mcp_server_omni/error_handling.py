"""Error handling and monitoring for Omni MCP Server.
...
def handle_omni_error(error: Exception, model: str = None, operation: str = None) -> MCPError:
    """Handle Omni-specific errors with appropriate categorization.

    Args:
        error: The exception from Omni
    ...
    # Check for specific Omni error patterns
    if "access denied" in error_str or "accessdenied" in error_str:
        ...
        return ConnectionError(
            f"Connection to Omni failed: {error}",
            context=context,
        )
    else:
        return SystemError(
            f"Omni error during {operation}: {error}",
            context=context,
        )
...
def format_user_error(error: MCPError) -> str:
    ...
    suggestions = {
        ...
        ErrorCategory.CONNECTION: "Unable to connect to Omni. Please check your connection settings.",
