"""Structured logging configuration for Omni MCP Server.
...
def setup_logging(
    ...
    # Create formatter
    if use_json or os.getenv("OMNI_MCP_LOG_JSON", "").lower() == "true":
        formatter = StructuredFormatter()
    else:
        format_string = log_format or os.getenv("OMNI_MCP_LOG_FORMAT", DEFAULT_FORMAT)
    ...
    if log_file or os.getenv("OMNI_MCP_LOG_FILE"):
        file_path = log_file or os.getenv("OMNI_MCP_LOG_FILE")
    ...
    logging.getLogger("mcp_server_omni").setLevel(numeric_level)
...
class LoggingConfig:
    ...
    def __init__(self):
        ...
        self.log_level = os.getenv("OMNI_MCP_LOG_LEVEL", "INFO")
        self.log_format = os.getenv("OMNI_MCP_LOG_FORMAT", DEFAULT_FORMAT)
        self.use_json = os.getenv("OMNI_MCP_LOG_JSON", "false").lower() == "true"
        self.log_file = os.getenv("OMNI_MCP_LOG_FILE")
        self.log_request_body = os.getenv("OMNI_MCP_LOG_REQUEST_BODY", "false").lower() == "true"
        self.log_response_body = os.getenv("OMNI_MCP_LOG_RESPONSE_BODY", "false").lower() == "true"
        self.slow_operation_threshold_ms = int(
            os.getenv("OMNI_MCP_SLOW_OPERATION_THRESHOLD_MS", "1000")
        )
...
perf_logger = PerformanceLogger(logging.getLogger("mcp_server_omni.performance"))
