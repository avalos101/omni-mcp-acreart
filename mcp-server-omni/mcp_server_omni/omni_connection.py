"""Omni XML-RPC connection management.

This module provides the OmniConnection class for managing connections
to Omni via XML-RPC using MCP-specific endpoints.
"""
...
class OmniConnectionError(Exception):
    """Base exception for Omni connection errors."""

    pass


class OmniConnection:
    """Manages XML-RPC connections to Omni with MCP-specific endpoints.

    This class provides connection management, health checking, and
    proper resource cleanup for Omni XML-RPC connections.
    """
    ...
    def __init__(
        ...
        Args:
            config: OmniConfig object with connection parameters
        ...
        logger.info(f"Initialized OmniConnection for {self._url_components['host']}")

    def _parse_url(self, url: str) -> Dict[str, Any]:
        """Parse and validate Omni URL.

        Args:
            url: The Omni server URL

        Returns:
            Dictionary with URL components

        Raises:
            OmniConnectionError: If URL is invalid
        """
        try:
            parsed = urlparse(url)
            ...
            if not parsed.hostname:
                raise OmniConnectionError("Invalid URL: missing hostname")
            ...
            raise OmniConnectionError(f"Failed to parse URL: {e}") from e
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            logger.info("Successfully connected to Omni server")

        except socket.timeout:
            raise OmniConnectionError(f"Connection timeout after {self.timeout} seconds") from None
            ...
            raise OmniConnectionError(f"Connection failed: {e}") from e

    def _test_connection(self) -> None:
        """Test connection by calling server_version.

        Raises:
            OmniConnectionError: If test fails
        """
        try:
            ...
            raise OmniConnectionError(f"Connection test failed: {e}") from e
    ...
    def disconnect(self, suppress_logging: bool = False) -> None:
        ...
        if not self._connected:
            if not suppress_logging:
                try:
                    logger.warning("Not connected to Omni")
        ...
        if not suppress_logging:
            try:
                logger.info("Disconnected from Omni server")
            except (ValueError, RuntimeError):
                # Ignore logging errors during cleanup
                pass
    ...
    def check_health(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            version = self._common_proxy.version()
            return True, f"Connected to Omni {version.get('server_version', 'unknown')}"
        except socket.timeout:
    ...
    def test_connection(self) -> bool:
        """Test if connection to Omni is working.
    ...
    def db_proxy(self) -> xmlrpc.client.ServerProxy:
        ...
        Raises:
            OmniConnectionError: If not connected
        """
        if not self._connected or not self._db_proxy:
            raise OmniConnectionError("Not connected to Omni")
    ...
    def list_databases(self) -> List[str]:
        """List all available databases on the Omni server.

        Returns:
            List of database names

        Raises:
            OmniConnectionError: If listing fails or not connected
        """
        if not self._connected:
            raise OmniConnectionError("Not connected to Omni")
        ...
        try:
            ...
            raise OmniConnectionError(f"Failed to list databases: {e}") from e
    ...
    def database_exists(self, db_name: str) -> bool:
        ...
        Raises:
            OmniConnectionError: If check fails
        """
        try:
            ...
            raise OmniConnectionError(f"Failed to check database existence: {e}") from e
    ...
    def auto_select_database(self) -> str:
        ...
        3. If multiple databases exist and one is named 'omni', use it
        ...
        Raises:
            OmniConnectionError: If no suitable database can be selected
        """
        ...
        try:
            ...
            raise OmniConnectionError(
                ...
                "Please specify OMNI_DB in your configuration."
            ) from e

        # Handle different scenarios
        if not databases:
            raise OmniConnectionError("No databases found on Omni server")
        ...
        # Multiple databases - check for 'omni'
        if "omni" in databases:
            logger.info("Auto-selected 'omni' database from multiple options")
            return "omni"

        # Cannot auto-select
        raise OmniConnectionError(
            f"Cannot auto-select database. Found {len(databases)} databases: "
            f"{', '.join(databases)}. Please specify OMNI_DB in configuration."
        )
    ...
    def validate_database_access(self, db_name: str) -> bool:
        ...
        Raises:
            OmniConnectionError: If validation fails
        """
        if not self._connected:
            raise OmniConnectionError("Not connected to Omni")
        ...
        try:
            ...
            raise OmniConnectionError("No authentication method configured")

        except xmlrpc.client.Fault as e:
            logger.error(f"XML-RPC fault validating database access: {e}")
            if "Access Denied" in str(e):
                return False
            raise OmniConnectionError(f"Failed to validate database access: {e}") from e
        except Exception as e:
    ...
    def _authenticate_api_key(self, database: str) -> bool:
        ...
        Raises:
            OmniConnectionError: If API request fails
        """
        ...
        try:
            ...
            if e.code == 401:
                ...
                raise OmniConnectionError(f"Failed to validate API key: {e}") from e
        except Exception as e:
    ...
    def _authenticate_password(self, database: str) -> bool:
        ...
        Raises:
            OmniConnectionError: If authentication fails
        """
        ...
        try:
            ...
            raise OmniConnectionError(f"Failed to authenticate: {e}") from e

    def authenticate(self, database: Optional[str] = None) -> None:
        """Authenticate with Omni using available credentials.
        ...
        raise OmniConnectionError("Authentication failed. Please check your credentials.")
    ...
    def execute(self, model: str, method: str, *args) -> Any:
        """Execute an operation on an Omni model.

        This is a simplified interface that calls execute_kw with empty kwargs.

        Args:
            model: The Omni model name (e.g., 'res.partner')
            method: The method to call (e.g., 'search', 'read')
            *args: Arguments to pass to the method

        Returns:
            The result from Omni

        Raises:
            OmniConnectionError: If not authenticated or execution fails
        """
    ...
    def execute_kw(self, model: str, method: str, args: List[Any], kwargs: Dict[str, Any]) -> Any:
        """Execute an operation on an Omni model with keyword arguments.

        This is the main method for interacting with Omni models via XML-RPC.
        ...
        if not self._authenticated:
            raise OmniConnectionError("Not authenticated. Call authenticate() first.")
        ...
        try:
            ...
            raise OmniConnectionError(f"Operation failed: {sanitized_message}") from e
        except socket.timeout:
            logger.error(f"Timeout during {method} on {model}")
            raise OmniConnectionError(f"Operation timeout after {self.timeout} seconds") from None
        except Exception as e:
    ...
    def search(self, model: str, domain: List[Union[str, List[Any]]], **kwargs) -> List[int]:
        ...
        Args:
            model: The Omni model name
            domain: Omni domain filter (e.g., [['is_company', '=', True]])
    ...
    def create(self, model: str, values: Dict[str, Any]) -> int:
        ...
        Raises:
            OmniConnectionError: If creation fails
        """
        try:
    ...
    def write(self, model: str, ids: List[int], values: Dict[str, Any]) -> bool:
        ...
        Raises:
            OmniConnectionError: If update fails
        """
        try:
    ...
    def unlink(self, model: str, ids: List[int]) -> bool:
        ...
        Raises:
            OmniConnectionError: If deletion fails
        """
        try:
    ...
    def get_server_version(self) -> Optional[Dict[str, Any]]:
        """Get Omni server version information.
...
@contextmanager
def create_connection(config: OmniConfig, timeout: int = OmniConnection.DEFAULT_TIMEOUT):
    """Create a connection context manager.

    Args:
        config: OmniConfig object
        timeout: Connection timeout in seconds

    Yields:
        Connected OmniConnection instance
    ...
    """
    conn = OmniConnection(config, timeout)
    try:
        conn.connect()
        yield conn
    finally:
