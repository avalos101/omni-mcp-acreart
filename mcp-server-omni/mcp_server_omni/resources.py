"""MCP resource handlers for Omni data access.

This module implements MCP resources for accessing Omni data through
...
class OmniResourceHandler:
    """Handles MCP resource requests for Omni data."""
    ...
    def _register_resources(self):
        ...
        @self.app.resource("omni://{model}/record/{record_id}")
        async def get_record(model: str, record_id: str) -> str:
            """Retrieve a specific record from Omni.
        ...
        @self.app.resource("omni://{model}/search")
        ...
        @self.app.resource("omni://{model}/count")
        ...
        @self.app.resource("omni://{model}/fields")
        ...
        try:
            with perf_logger.track_operation("resource_get_record", model=model):
                ...
                if not self.connection.is_authenticated:
                    raise ValidationError("Not authenticated with Omni", context=context)
    ...
    async def _handle_search(
        ...
        try:
            ...
            # Ensure we're connected
            if not self.connection.is_authenticated:
                raise ResourceError("Not authenticated with Omni")
...
def register_resources(
    ...
    """Register all Omni resources with the FastMCP app.
    ...
    """
    handler = OmniResourceHandler(app, connection, access_controller, config)
    logger.info("Registered Omni MCP resources")
    return handler
