"""MCP Server implementation for Omni.

This module provides the FastMCP server that exposes Omni data
...
from .config import OmniConfig, get_config
...
class OmniMCPServer:
    """Main MCP server class for Omni integration.

    This class manages the FastMCP server instance and maintains
    the connection to Omni. The server lifecycle is managed by
    establishing connection before starting and cleaning up on exit.
    """

    def __init__(self, config: Optional[OmniConfig] = None):
        """Initialize the Omni MCP server.

        Args:
            config: Optional OmniConfig instance. If not provided,
        ...
        self.connection: Optional[OmniConnection] = None
        ...
        self.app = FastMCP(
            ...
            instructions="MCP server for accessing and managing Omni ERP data through the Model Context Protocol",
        )

        logger.info(f"Initialized Omni MCP Server v{SERVER_VERSION}")

    def _ensure_connection(self):
        """Ensure connection to Omni is established.
        ...
        if not self.connection:
            try:
                logger.info("Establishing connection to Omni...")
                with perf_logger.track_operation("connection_setup"):
                    ...
                    self.connection = OmniConnection(
                ...
                logger.info(f"Successfully connected to Omni at {self.config.url}")
                ...
                if isinstance(e, (OmniConnectionError, ConfigurationError)):
                    raise
                # Handle other unexpected errors
                error_handler.handle_error(e, context=context)

    def _cleanup_connection(self):
        """Clean up Omni connection."""
        if self.connection:
            try:
                logger.info("Closing Omni connection...")
    ...
    def _setup_handlers(self):
        ...
        - Resource handlers for Omni data access
        - Tool handlers for Omni operations
    ...
    async def run_stdio(self):
        ...
        except (OmniConnectionError, ConfigurationError):
    ...
    def get_capabilities(self) -> Dict[str, Dict[str, bool]]:
        ...
        return {
            "capabilities": {
                "resources": True,  # Exposes Omni data as resources
                "tools": True,  # Provides tools for Omni operations
