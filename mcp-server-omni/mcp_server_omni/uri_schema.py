"""URI schema module for Omni MCP Server.

This module implements the URI schema for accessing Omni data through MCP resources.
It provides parsing, validation, and building utilities for Omni URIs following the
pattern: omni://{model}/{operation}?{parameters}
...
class OmniOperation(Enum):
    """Supported Omni URI operations."""
...
@dataclass
class OmniURI:
    """Parsed Omni URI representation."""

    model: str
    operation: OmniOperation
...
# URI pattern for matching omni:// URIs
...
URI_PATTERN = re.compile(r"^omni://([^/]+)/([^/?]+)(?:/(\d+))?(?:\?(.*))?$")


def parse_uri(uri: str) -> OmniURI:
    """Parse an Omni URI string into its components.

    Args:
        uri: The URI string to parse (e.g., "omni://res.partner/search?domain=[('is_company','=',True)]")

    Returns:
        OmniURI object with parsed components
    ...
    if not uri.startswith("omni://"):
        raise URIParseError(f"URI must start with 'omni://', got: {uri}")
    ...
    try:
        if operation_str == "record" and record_id_str:
            operation = OmniOperation.RECORD
            record_id = int(record_id_str)
        elif operation_str in [op.value for op in OmniOperation]:
            operation = OmniOperation(operation_str)
            record_id = None
        else:
    ...
    if operation == OmniOperation.RECORD and not record_id:
    ...
    if operation == OmniOperation.BROWSE and not ids:
...
def build_uri(
    ...
    """Build an Omni URI from components.
    ...
    Args:
        ...
        domain: Omni domain expression (should be URL-encoded if needed)
    ...
    # Validate operation
    if operation not in [op.value for op in OmniOperation]:
    ...
    if operation == "record":
        if not record_id:
            raise URIValidationError("Record operation requires an ID")
        uri = f"omni://{model}/record/{record_id}"
    else:
        uri = f"omni://{model}/{operation}"
