...
class MCPModelManager:
    ...
    def __init__(self, config: OmniConfig):
        ...
        Args:
            config: Omni configuration with admin credentials
        """
        self.config = config
        self.connection = OmniConnection(config)
    ...
    def _ensure_model_enabled(self, model_perm: ModelPermissions) -> None:
        ...
        if existing_ids:
            ...
            if model_id:
                ...
                logger.warning(f"Model not found in Omni: {model_perm.model}")
...
def create_admin_config() -> OmniConfig:
    """Create admin configuration for managing MCP models.

    Returns:
        OmniConfig with admin credentials
    """
