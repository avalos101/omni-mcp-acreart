...
from mcp_server_omni.server import OmniMCPServer
...
class MCPTestServer:
    ...
    def __init__(self, config: Optional[OmniConfig] = None):
        ...
        self.server: Optional[OmniMCPServer] = None
        self.omni_connection: Optional[OmniConnection] = None

    async def start(self) -> None:
        """Start the MCP server for testing."""
        # Create server instance
        self.server = OmniMCPServer(self.config)
        ...
        self.omni_connection = self.server.connection
    ...
    def start_subprocess(self) -> subprocess.Popen:
        ...
        env.update(
            {
                "OMNI_URL": self.config.url,
                "OMNI_DATABASE": self.config.database,
                "PYTHONPATH": str(Path(__file__).parent.parent.parent),
            }
        )

        if self.config.api_key:
            env["OMNI_API_KEY"] = self.config.api_key
        else:
            if self.config.username:
                env["OMNI_USERNAME"] = self.config.username
            if self.config.password:
                env["OMNI_PASSWORD"] = self.config.password

        # Start server subprocess
        self.server_process = subprocess.Popen(
            [sys.executable, "-m", "mcp_server_omni"],
...
def mcp_test_server(config: Optional[OmniConfig] = None) -> Generator[MCPTestServer, None, None]:
...
class OmniTestData:
    """Helper class for managing test data in Omni."""

    def __init__(self, connection: OmniConnection):
...
def check_omni_health(base_url: str, api_key: str) -> bool:
    """Check if Omni MCP endpoints are healthy."""
    try:
...
async def run_mcp_command(
    server: OmniMCPServer, command: str, params: Dict[str, Any]
) -> Dict[str, Any]:
...
async def validate_resource_operation(
    server: OmniMCPServer, uri: str, expected_type: str = "text/plain"
...
def create_test_env_file(test_dir: Path) -> Path:
    ...
    if not os.getenv("OMNI_URL"):
        raise ValueError("OMNI_URL environment variable not set. Please configure .env file.")

    if not os.getenv("OMNI_API_KEY"):
        raise ValueError("OMNI_API_KEY environment variable not set. Please configure .env file.")

    env_content = f"""
OMNI_URL={os.getenv("OMNI_URL")}
OMNI_API_KEY={os.getenv("OMNI_API_KEY")}
OMNI_DATABASE={os.getenv("OMNI_DB")}
OMNI_MCP_LOG_LEVEL={os.getenv("OMNI_MCP_LOG_LEVEL", "INFO")}
"""
