...
from mcp_server_omni.resources import OmniResourceHandler, register_resources
...
@pytest.fixture
def mock_connection():
    """Create mock OmniConnection."""
    conn = Mock(spec=OmniConnection)
...
def resource_handler(mock_app, mock_connection, mock_access_controller, mock_config):
    """Create OmniResourceHandler instance."""
    return OmniResourceHandler(mock_app, mock_connection, mock_access_controller, mock_config)


class TestOmniResourceHandler:
    """Test OmniResourceHandler functionality."""

    def test_init(self, mock_app, mock_connection, mock_access_controller, mock_config):
        """Test handler initialization."""
        handler = OmniResourceHandler(
    ...
    async def test_handle_record_retrieval_not_authenticated(
        ...
        assert "Not authenticated with Omni" in str(exc_info.value)
    ...
    async def test_handle_record_retrieval_connection_error(
        ...
        mock_connection.search.side_effect = OmniConnectionError("Connection lost")
...
class TestRegisterResources:
    ...
    def test_register_resources(
        ...
        assert isinstance(handler, OmniResourceHandler)
        assert handler.app == mock_app
        assert handler.connection == mock_connection
        assert handler.access_controller == mock_access_controller
        assert handler.config == mock_config
...
class TestResourceIntegration:
    ...
    async def test_real_record_retrieval(self, test_config):
        ...
        connection = OmniConnection(test_config)
        connection.connect()
