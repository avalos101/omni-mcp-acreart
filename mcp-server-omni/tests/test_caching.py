"""Tests for caching functionality with Omni integration."""
...
from mcp_server_omni.config import OmniConfig, load_config
from mcp_server_omni.omni_connection import OmniConnection
from mcp_server_omni.performance import PerformanceManager

# Import skip_on_rate_limit decorator
from .test_xmlrpc_operations import skip_on_rate_limit


class TestOmniConnectionCaching:
    """Test caching functionality in OmniConnection."""

    @pytest.fixture
    def mock_config(self):
        """Create mock config."""
        config = Mock(spec=OmniConfig)
        config.url = os.getenv("OMNI_URL", "http://localhost:8069")
        config.db = "test"
    ...
    def test_fields_get_caching(self, mock_config, mock_performance_manager):
        ...
        conn = OmniConnection(mock_config, performance_manager=mock_performance_manager)
...
class TestCachingIntegration:
    """Integration tests for caching with real Omni connection."""
    ...
    def test_real_fields_caching(self, real_config, performance_manager):
        """Test field caching with real Omni connection."""
        conn = OmniConnection(real_config, performance_manager=performance_manager)
    ...
    def test_real_record_caching(self, real_config, performance_manager):
        """Test record caching with real Omni connection."""
        conn = OmniConnection(real_config, performance_manager=performance_manager)
    ...
    def test_cache_memory_limits(self, real_config):
        ...
        conn = OmniConnection(real_config, performance_manager=small_manager)
    ...
    def test_connection_pool_reuse(self, real_config, performance_manager):
        ...
        conn1 = OmniConnection(real_config, performance_manager=performance_manager)
        conn2 = OmniConnection(real_config, performance_manager=performance_manager)
