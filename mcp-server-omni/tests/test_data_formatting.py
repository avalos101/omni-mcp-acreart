...
from mcp_server_omni.config import get_config
from mcp_server_omni.formatters import DatasetFormatter, RecordFormatter
...
class TestRecordFormatter:
    ...
    def test_format_many2one_field(self, formatter):
        ...
        assert "partner_id: Parent Company (omni://res.partner/record/10)" in result
    ...
    def test_format_one2many_field(self, formatter):
        ...
        assert "→ View all: omni://res.partner/search?" in result
    ...
    def test_format_many2many_field(self, formatter):
        ...
        assert "omni://res.partner.category/search?domain" in result
    ...
    def test_format_datetime_field(self, formatter):
        ...
        record = {
            ...
            "datetime_compact": "20240115T14:30:00",  # Omni compact format
...
class TestDatasetFormatter:
    ...
    def test_format_search_with_pagination(self, formatter):
        ...
        result = formatter.format_search_results(
            ...
            current_page=2,
            total_pages=3,
            prev_uri="omni://res.partner/search?limit=10&offset=0",
            next_uri="omni://res.partner/search?limit=10&offset=20",
        )
        ...
        assert "← Previous page: omni://res.partner/search?limit=10&offset=0" in result
        assert "→ Next page: omni://res.partner/search?limit=10&offset=20" in result
...
class TestFormattingIntegration:
    """Integration tests with real Omni data."""

    @pytest.mark.integration
    def test_format_real_partner_record(self):
        """Test formatting real partner records from Omni."""
        config = get_config()
    ...
    def test_format_real_search_results(self):
        """Test formatting real search results from Omni."""
        config = get_config()
        connection = OmniConnection(config)
        ...
        try:
            ...
            next_uri = "omni://res.partner/search?limit=5&offset=5" if total > 5 else None
    ...
    def test_format_various_field_types(self):
        """Test formatting various Omni field types."""
        config = get_config()
        connection = OmniConnection(config)
