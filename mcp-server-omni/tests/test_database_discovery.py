"""Tests for database discovery functionality in OmniConnection.
...
from mcp_server_omni.omni_connection import OmniConnection, OmniConnectionError


def is_omni_server_running(host="localhost", port=8069):
    """Check if Omni server is running."""
...
class TestDatabaseDiscovery:
    ...
    def connection(self, config):
        """Create OmniConnection instance."""
        return OmniConnection(config)
    ...
    def test_list_databases_success(self, connection):
        ...
        mock_proxy = Mock()
        mock_proxy.list.return_value = ["db1", "db2", os.getenv("OMNI_DB", "db")]
        connection._db_proxy = mock_proxy

        databases = connection.list_databases()

        assert databases == ["db1", "db2", os.getenv("OMNI_DB", "db")]
    ...
    def test_list_databases_error(self, connection):
        ...
        with pytest.raises(OmniConnectionError, match="Failed to list databases"):
            connection.list_databases()
    ...
    def test_database_exists_true(self, connection):
        ...
        connection._connected = True
        mock_proxy = Mock()
        mock_proxy.list.return_value = ["db1", os.getenv("OMNI_DB", "db"), "test"]
        connection._db_proxy = mock_proxy

        assert connection.database_exists(os.getenv("OMNI_DB", "db")) is True
    ...
    def test_database_exists_false(self, connection):
        ...
        connection._connected = True
        mock_proxy = Mock()
        mock_proxy.list.return_value = ["db1", os.getenv("OMNI_DB", "db")]
        connection._db_proxy = mock_proxy
    ...
    def test_auto_select_configured_database(self):
        ...
        config = OmniConfig(url="http://localhost:8069", api_key="test_api_key", database="mydb")
        connection = OmniConnection(config)
        connection._connected = True
        mock_proxy = Mock()
    ...
    def test_auto_select_multiple_with_omni(self, connection):
        """Test auto-selection prefers 'omni' database when multiple exist."""
        connection._connected = True
        mock_proxy = Mock()
        mock_proxy.list.return_value = ["db1", "omni", "test", "prod"]
        connection._db_proxy = mock_proxy

        selected = connection.auto_select_database()

        assert selected == "omni"

    def test_auto_select_multiple_without_omni(self, connection):
        """Test auto-selection fails with multiple databases and no 'omni'."""
        connection._connected = True
        mock_proxy = Mock()
        mock_proxy.list.return_value = ["db1", "test", "prod"]
        connection._db_proxy = mock_proxy

        with pytest.raises(OmniConnectionError, match="Cannot auto-select"):
    ...
    def test_auto_select_no_databases(self, connection):
        ...
        with pytest.raises(OmniConnectionError, match="No databases found"):
            connection.auto_select_database()
    ...
    def test_validate_database_access_api_key(self, connection):
        ...
        connection._connected = True
        mock_proxy = Mock()
        mock_proxy.list.return_value = [os.getenv("OMNI_DB", "db"), "test"]
        connection._db_proxy = mock_proxy

        # Should just check existence for API key auth
        assert connection.validate_database_access(os.getenv("OMNI_DB", "db")) is True
    ...
    def test_validate_database_access_credentials(self):
        ...
        mock_common.authenticate.assert_called_once_with(
            os.getenv("OMNI_DB", "db"),
            os.getenv("OMNI_USER", "admin"),
            os.getenv("OMNI_PASSWORD", "admin"),
            {},
        )
    ...
    def test_validate_database_access_auth_failed(self):
        ...
        assert connection.validate_database_access(os.getenv("OMNI_DB", "db")) is False
...
@pytest.mark.skipif(
    not is_omni_server_running(), reason="Omni server not running at localhost:8069"
)
class TestDatabaseDiscoveryIntegration:
    ...
    def real_config(self):
        """Create configuration for real Omni server."""
    ...
    def test_real_list_databases(self, real_config):
        """Test listing databases on real Omni server."""
        with OmniConnection(real_config) as conn:
    ...
    def test_real_auto_select(self, real_config):
        """Test auto-selection on real Omni server."""
        with OmniConnection(real_config) as conn:
