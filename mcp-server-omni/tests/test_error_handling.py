...
from mcp_server_omni.logging_config import (
    LoggingConfig,
    PerformanceLogger,
...
class TestOmniErrorHandling:
    """Test Omni-specific error handling."""

    def test_handle_omni_access_denied(self):
        """Test handling Omni access denied errors."""
        error = Exception("Access Denied for model res.partner")
        result = handle_omni_error(error, model="res.partner", operation="read")
    ...
    def test_handle_omni_not_found(self):
        """Test handling Omni not found errors."""
        error = Exception("Record does not exist")
        result = handle_omni_error(error, model="res.partner")

        assert isinstance(result, NotFoundError)
        assert "Resource not found: res.partner" in result.message

    def test_handle_omni_validation(self):
        """Test handling Omni validation errors."""
        error = Exception("Invalid field value")
        result = handle_omni_error(error, operation="create")

        assert isinstance(result, ValidationError)
        assert "Validation failed for create" in result.message

    def test_handle_omni_connection(self):
        """Test handling Omni connection errors."""
        error = Exception("Connection timeout")
        result = handle_omni_error(error)

        assert isinstance(result, ConnectionError)
        assert "Connection to Omni failed" in result.message

    def test_handle_omni_generic(self):
        """Test handling generic Omni errors."""
        error = Exception("Some other error")
        result = handle_omni_error(error, operation="search")

        assert isinstance(result, SystemError)
        assert "Omni error during search" in result.message
...
class TestUserErrorFormatting:
    ...
    def test_format_connection_error(self):
        ...
        assert "Unable to connect to Omni" in formatted
    ...
    def test_logging_config_from_env(self):
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "OMNI_MCP_LOG_JSON": "true",
                "OMNI_MCP_LOG_FILE": "/tmp/test.log",
                "OMNI_MCP_SLOW_OPERATION_THRESHOLD_MS": "500",
            },
        ):
