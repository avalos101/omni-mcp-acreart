...
from mcp_server_omni.access_control import AccessControlError
from mcp_server_omni.error_handling import ValidationError as ToolError
from mcp_server_omni.omni_connection import OmniConnectionError
from mcp_server_omni.resources import OmniResourceHandler
from mcp_server_omni.tools import OmniToolHandler
...
class TestErrorSanitizationIntegration:
    ...
    def tool_handler(self):
        ...
        return OmniToolHandler(app, connection, access_controller, config)
    ...
    async def test_tool_xmlrpc_fault_sanitization(self, tool_handler):
        ...
        tool_handler.connection.search_count.side_effect = OmniConnectionError(
    ...
    async def test_tool_connection_error_sanitization(self, tool_handler):
        ...
        tool_handler.connection.is_authenticated = True
        tool_handler.connection.read.side_effect = OmniConnectionError(
            "Operation failed: Cannot connect to Omni server"
        )
    ...
    async def test_tool_generic_error_sanitization(self, tool_handler):
        ...
        tool_handler.connection.search.side_effect = Exception(
            ...
            '  File "/opt/omni/models.py", line 123, in execute\n'
        ...
        assert "/opt/omni" not in error_msg
    ...
    async def test_resource_error_sanitization(self, resource_handler):
        ...
        from mcp_server_omni.error_handling import NotFoundError
    ...
    async def test_mcp_error_response_sanitization(self, tool_handler):
        ...
        from mcp_server_omni.error_handling import ValidationError
    ...
    def test_complex_error_chain_sanitization(self):
        ...
        from mcp_server_omni.error_sanitizer import ErrorSanitizer
        ...
        File "/opt/omni/addons/mcp_server/controllers/xmlrpc.py", line 123, in execute_kw
        File "/usr/lib/python3.10/xmlrpc/client.py", line 1122, in __call__
        xmlrpc.client.Fault: <Fault 1: 'Traceback (most recent call last):
          File "/opt/omni/omni/http.py", line 1589, in _serve_db
        omni.exceptions.ValidationError: Invalid field res.partner.invalid_field'>
        """
        ...
        assert "/opt/omni" not in sanitized
