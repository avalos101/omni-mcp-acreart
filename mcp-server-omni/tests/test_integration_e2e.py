"""Comprehensive end-to-end integration tests for Omni MCP Server.

These tests validate the complete MCP server functionality with a real Omni server
...
from mcp_server_omni.omni_connection import OmniConnection
...
# Mark all tests in this module as integration tests requiring Omni
pytestmark = [pytest.mark.integration, pytest.mark.omni_required]
...
class TestServerLifecycle:
    ...
    async def test_server_startup_and_shutdown(self):
        ...
        assert server.omni_connection is not None

        # Verify connection is active
        assert server.omni_connection.is_connected

        # Stop server
        await server.stop()
        assert server.server is None
        assert server.omni_connection is None
    ...
    def test_server_with_env_file(self, tmp_path):
        ...
        try:
            ...
            assert config.api_key == os.getenv("OMNI_API_KEY")
            assert config.database == os.getenv("OMNI_DB")
    ...
    def test_uvx_server_startup(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "mcp_server_omni", "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
class TestAuthenticationFlows:
    ...
    def test_username_password_fallback(self):
        ...
        config = OmniConfig(
            ...
            password=os.getenv("OMNI_PASSWORD", "admin"),
        )
...
class TestResourceOperations:
    """Test all resource operations with real Omni data."""
    ...
    async def test_record_resource_operation(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://res.users/record?id=2"
    ...
    async def test_search_resource_operation(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://res.users/search?domain=[('id','=',2)]&limit=5"
            success, error = await validate_resource_operation(server.server, uri)
    ...
    async def test_browse_resource_operation(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://res.partner/browse?offset=0&limit=10"
            success, error = await validate_resource_operation(server.server, uri)
    ...
    async def test_count_resource_operation(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://res.partner/count?domain=[]"
            success, error = await validate_resource_operation(server.server, uri)
    ...
    async def test_resource_with_complex_domain(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = f"omni://res.users/search?domain={domain}&limit=5"
...
class TestErrorHandling:
    ...
    async def test_invalid_model_error(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://invalid.model/search?domain=[]"
            success, error = await validate_resource_operation(server.server, uri)
    ...
    async def test_access_denied_error(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://ir.config_parameter/search?domain=[]"
            response = await run_mcp_command(server.server, "resources/read", {"uri": uri})
    ...
    async def test_invalid_uri_format_error(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            invalid_uris = [
                "invalid://format",
                "omni://",
                "omni://model/invalid_operation",
                "omni://res.partner/search?invalid_param=test",
            ]
    ...
    async def test_large_result_handling(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uri = "omni://res.partner/browse?limit=1000"
...
class TestPerformanceAndReliability:
    ...
    async def test_operation_performance(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            operations = [
                ("Record fetch", "omni://res.users/record?id=2", 1.0),
                ("Small search", "omni://res.partner/search?limit=10", 1.0),
                ("Field metadata", "omni://res.partner/fields", 2.0),
                ("Count operation", "omni://res.partner/count", 1.0),
            ]
    ...
    async def test_concurrent_operations(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            uris = [
                "omni://res.users/record?id=2",
                "omni://res.partner/count",
                "omni://res.partner/fields",
                "omni://res.users/search?limit=5",
            ]
    ...
    def test_server_health_monitoring(self):
        ...
        # Check Omni health
        is_healthy = check_omni_health(config.url, config.api_key)
        assert is_healthy

        # Test with invalid credentials
        is_healthy = check_omni_health(config.url, "invalid_key")
    ...
    async def test_mcp_response_format(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            response = await run_mcp_command(
                server.server, "resources/read", {"uri": "omni://res.users/record?id=2"}
            )
...
class TestEndToEndWorkflow:
    ...
    async def test_read_workflow(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            # Test search operation
            search_uri = "omni://res.users/search?domain=[('id','=',2)]"
            response = await run_mcp_command(server.server, "resources/read", {"uri": search_uri})

            assert "result" in response

            # Test browse operation
            browse_uri = "omni://res.users/browse?limit=5"
    ...
    async def test_relationship_navigation_workflow(self):
        ...
        async with MCPTestServer(config) as server:
            ...
            # Start with a user
            user_uri = "omni://res.users/record?id=2"
            ...
            partner_uri = "omni://res.partner/search?domain=[('user_ids','=',2)]"
