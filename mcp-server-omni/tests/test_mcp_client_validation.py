...
# Test configuration
TEST_CONFIG = {
    "OMNI_URL": os.getenv("OMNI_URL", "http://localhost:8069"),
    "OMNI_DB": os.getenv("OMNI_DB"),
...
class TestMCPProtocolCompliance:
    ...
    async def test_resource_listing(self, test_env):
        ...
        async with client.connect() as connected_client:
            ...
            for resource in resources[:5]:  # Check first 5
                ...
                assert resource.uri.startswith("omni://")
            ...
            if search_result.content and len(search_result.content) > 0:
                ...
                if isinstance(content, TextContent):
                    ...
                    if "ID:" in text:
                        ...
                        # Read the resource
                        uri = f"omni://res.partner/record/{record_id}"
    ...
    async def test_read_resource_not_found(self, test_env):
        ...
        async with client.connect() as connected_client:
            ...
            uri = "omni://res.partner/record/999999"
...
class TestMCPIntegration:
    ...
    async def test_end_to_end_workflow(self, test_env):
        ...
        async with client.connect() as connected_client:
            ...
            try:
                ...
                record_content = await connected_client.read_resource("omni://res.partner/record/1")
    ...
    async def test_error_handling_workflow(self, test_env):
        ...
        async with client.connect() as connected_client:
            ...
            with pytest.raises(McpError):
                await connected_client.read_resource("omni://res.partner/record/999999999")
...
# Test with real Omni server if available
@pytest.mark.integration
class TestRealOmniServer:
    """Test with real Omni server."""

    @pytest.mark.asyncio
    async def test_real_server_connection(self):
        """Test connection to real Omni server."""
        # Skip if no real server
        try:
            import urllib.request

            with urllib.request.urlopen(
                f"{os.getenv('OMNI_URL', 'http://localhost:8069')}/mcp/health", timeout=2
            ) as response:
        ...
        async with client.connect() as connected_client:
            ...
            try:
                content = await connected_client.read_resource("omni://res.partner/search?limit=1")
                assert isinstance(content, str)
