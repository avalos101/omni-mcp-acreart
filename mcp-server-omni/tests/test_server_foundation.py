...
lifecycle management, and connection to Omni.
"""
...
from mcp_server_omni.omni_connection import OmniConnectionError
from mcp_server_omni.server import SERVER_VERSION, OmniMCPServer
...
class TestServerFoundation:
    ...
    def server_with_mock_connection(self, valid_config):
        ...
        with patch("mcp_server_omni.server.OmniConnection") as mock_conn_class:
            ...
            server = OmniMCPServer(valid_config)
    ...
    def test_server_initialization(self, valid_config):
        ...
        assert server.app.name == "omni-mcp-server"
    ...
    def test_server_initialization_with_env_config(self, monkeypatch, tmp_path):
        ...
        from mcp_server_omni.config import reset_config

        reset_config()

        # Set up environment variables
        monkeypatch.setenv("OMNI_URL", "http://test.omni.com")
        monkeypatch.setenv("OMNI_API_KEY", "env_test_key")
        monkeypatch.setenv("OMNI_DB", "env_test_db")

        try:
            # Create server without explicit config
            server = OmniMCPServer()

            assert server.config.url == "http://test.omni.com"
    ...
    def test_ensure_connection_failure(self, server_with_mock_connection):
        ...
        server._mock_connection.connect.side_effect = OmniConnectionError("Connection failed")

        # Ensure connection should raise an error
        with pytest.raises(OmniConnectionError, match="Connection failed"):
            server._ensure_connection()
    ...
    async def test_run_stdio_success(self, server_with_mock_connection):
        ...
        with patch("mcp_server_omni.server.AccessController") as mock_access_ctrl:
            with patch("mcp_server_omni.server.register_resources") as mock_register:
                mock_handler = Mock()
    ...
    async def test_run_stdio_connection_failure(self, server_with_mock_connection):
        ...
        # Make connection fail
        server._mock_connection.connect.side_effect = OmniConnectionError("Failed to connect")

        # Should raise an error
        with pytest.raises(OmniConnectionError, match="Failed to connect"):
            await server.run_stdio()
...
class TestServerIntegration:
    ...
    def test_server_with_env_file(self, tmp_path, monkeypatch):
        ...
        from mcp_server_omni.config import load_config, reset_config
        ...
        env_file.write_text(
            """
OMNI_URL=http://localhost:8069
OMNI_API_KEY=test_integration_key
OMNI_DB=test_integration_db
...
"""
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_real_omni_connection(self):
        """Test with real Omni connection using .env credentials.

        This test requires a running Omni server with valid credentials
        in the .env file.
        """
        ...
        if not os.getenv("OMNI_URL"):
            pytest.skip("OMNI_URL not set in environment")
        ...
        except OmniConnectionError as e:
            # Connection errors are expected if Omni is not running
            pytest.skip(f"Integration test skipped (Omni not available): {e}")
...
class TestMainEntry:
    ...
    def test_version_flag(self, capsys):
        ...
        assert f"omni-mcp-server v{SERVER_VERSION}" in version_output
    ...
    def test_main_with_invalid_config(self, capsys, monkeypatch):
        ...
        # Set invalid config
        monkeypatch.setenv("OMNI_URL", "")  # Empty URL
    ...
    def test_main_with_valid_config(self, monkeypatch):
        ...
        monkeypatch.setenv("OMNI_API_KEY", "test_key")

        # Mock the server and its run_stdio method
        with patch("mcp_server_omni.__main__.OmniMCPServer") as mock_server_class:
            mock_server = Mock()
...
class TestFastMCPApp:
    ...
    def test_fastmcp_app_creation(self, valid_config):
        ...
        assert server.app is not None
        assert server.app.name == "omni-mcp-server"
        assert "Omni ERP data" in server.app.instructions
