"""Tests for OmniConnection write operations (create, write, unlink)."""
...
class TestWriteOperations:
    """Test write operations in OmniConnection."""
    ...
    def connection(self, mock_config, mock_performance_manager):
        """Create OmniConnection with mocks."""
    ...
    def test_create_record_error(self, connection):
        ...
        with patch.object(
            connection, "execute_kw", side_effect=OmniConnectionError("Create failed")
        ):
            with pytest.raises(OmniConnectionError, match="Create failed"):
    ...
    def test_write_records_error(self, connection):
        ...
        with patch.object(
            connection, "execute_kw", side_effect=OmniConnectionError("Write failed")
        ):
            with pytest.raises(OmniConnectionError, match="Write failed"):
    ...
    def test_unlink_records_error(self, connection):
        ...
        with patch.object(
            connection, "execute_kw", side_effect=OmniConnectionError("Delete failed")
        ):
            with pytest.raises(OmniConnectionError, match="Delete failed"):
    ...
    def test_write_operations_not_authenticated(self, connection):
        ...
        with pytest.raises(OmniConnectionError, match="Not authenticated"):
            connection.create("res.partner", {"name": "Test"})
